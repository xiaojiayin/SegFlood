#!/usr/bin/env bash
# Follow-up to submit_ood_occlusion_evals.sh: 30 % occlusion with IN-RANGE constant fills on S1S2-Water
# (TOA reflectance DN/1e4): 0.6 = bright, cloud-like; 0.05 = dark, shadow-like. Inference only.
PROJECT_ROOT="${PROJECT_ROOT:-$(git -C "$(dirname "${BASH_SOURCE[0]}")" rev-parse --show-toplevel 2>/dev/null || pwd)}"
set -euo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/../../.."
L=${PROJECT_ROOT}/logs
STAMP="$(date +%Y%m%d_%H%M%S)"; MANIFEST="scripts/run/jag_supplement/submissions_ood2_occlusion_${STAMP}.tsv"
...
declare -A COND=([occ30_cloud]="0.3 0.6" [occ30_shadow]="0.3 0.05")
for m in inputconcat add concat bixattn gate mastd oa_s42 oa_s123 oa_s2026 noalign_s42 noalign_s123 noalign_s2026; do
  ...
  for c in occ30_cloud occ30_shadow; do
    read -r st fill <<< "${COND[$c]}"
    name="jag27_ood2_${m}_${c}"
    ovr="${OVR[$m]} +model.test_degradation_seed=1 +model.test_degradation_fill=${fill}"
    jid="$(sbatch --exclude g59,g10 --time=00:40:00 --job-name "${name}" scripts/run/jag_supplement/eval_robustness.sh resnet50_resnet50_s1s2water "${CKPT[$m]}" "${name}" dual optical_cloud "${st}" "${ovr}" | awk '{print $4}')"
